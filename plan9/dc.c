...
#define Bprint fprintf
#define Bflush fflush
#define Bgetc fgetc
#define Bputc fputc
...
#define zero(p)		for(pp=(p)->beg;pp<(p)->last;)\
				*pp++='\0'
#define OUTC(x)		{Bputc(x,bout); if(--count == 0){Bprint(bout,"\\\n"); count=ll;} }
...
command(void)
{
	...
	default:
		...
		*sl = 0;
		if((pid = fork()) == 0) {
			execl("/bin/sh","sh","-c",line,nil);
			exit(1);
		}
		for(;;) {
			if((p = waitpid(-1,&c,0)) < 0)
				break;
			if(p== pid)
				break;
		}
...
sdump(char *s1, Blk *hptr)
{
	...
	Bprint(bout,"%s %p rd %p wt %p beg %p last %p\n",
		s1,(void*)hptr,hptr->rd,hptr->wt,hptr->beg,hptr->last);
	p = hptr->beg;
	while(p < hptr->wt)
		Bprint(bout,"%d ",*p++);
	Bprint(bout,"\n");
